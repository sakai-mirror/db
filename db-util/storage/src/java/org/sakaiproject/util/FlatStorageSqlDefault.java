...
public class FlatStorageSqlDefault implements FlatStorageSql
{
   ...
   public String getInsertSql(String table, String idField, String extraIdField) {
      return "insert into " + table + "( " + idField + ", NAME, VALUE" + ((extraIdField != null) ? (", " + extraIdField) : "") + " ) values (?,?,?" + ((extraIdField != null) ? ",?" : "") + ")";
   }
   ...
   public String getSelectCount2Sql(String table, String join, String where) {
      return "select count(1) from " + table + ((join == null) ? "" : ("," + join)) + (((where != null) && (where.length() > 0)) ? (" where " + where) : "");
   }
   ...
   public String getSelectFieldsSql1(String table, String fieldList, String idField, String sortField1, String sortField2, int begin, int end) {
      return "select limit " + begin + " " + end + " " + fieldList + " from " + table + " order by " + table + "." + sortField1 + (sortField2 == null ? "" : "," + table + "." + sortField2);
   }
   ...
   public String getSelectFieldsSql3(String table, String fieldList, String idField, String sortField1, String sortField2, int begin, int end, String join, String where, String order) {
      return "select limit " + begin + " " + end + " " + fieldList  + " from " + table + ((join == null) ? "" : ("," + join)) + (((where != null) && (where.length() > 0)) ? (" where " + where) : "") +
             " order by " + order + "," + table + "." + sortField1 + (sortField2 == null ? "" : "," + table + "." + sortField2);
   }
